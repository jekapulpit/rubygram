...
export async function receiveInvite(data) {
  if (window.location.pathname === '/home/notifications') readInvites();
  else syncCurrentUser();
  if (data.invite.user_id === getCurrentUser().id) store.dispatch({ type: data.type, invite: data.invite });
}
