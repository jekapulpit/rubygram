...
export function receiveMessage(action) {
  ...
  switch (action.type) {
    case rooms.UNSUBSCRIBE:
      if (action.user_id === getCurrentUser().id) window.location = '/home/rooms';
      break;
    case rooms.ANSWER:
      store.dispatch({ type: search.UPDATE_USER_RESULTS, result: action.user });
      break;
    case messages.RECEIVE:
      if (window.location.pathname === (`/home/rooms/${action.message.recipient_id}`)) readAllMessages(action.message.recipient_id);
      basicScroll();
      break;
  }
}
